...
struct cfg_item {
	const char *cfg_item_name;//项目名称
	enum ITEM_TYPE cfg_item_type;//项目类型（枚举）
	union cfg_value cfg_item_value;//项目的值（公用体）
};
...
{
	item[0].cfg_item_value.ivalue = 123;//item的成员cfg_item_value是共用体，.ivalue表示是整数
	item[1].cfg_item_value.dvalue = 123.45;//double
	strcpy(item[2].cfg_item_value.svalue, "Hello");//char[]
	item[3].cfg_item_value.cvalue = 'Y';//char 
//	item[4].cfg_item_value.ivalue = 0;
	item[5].cfg_item_value.ivalue = 99999;//int
	strcpy(item[6].cfg_item_value.svalue, "Hello<String>");//char[]
...
{
	CFT fcfg;//定义了一个CFT类，fcfg 用此类的函数来执行文件操作
	int ret, i;
	char tmp[256];
	int judge=0;
	/* 第一次模拟，添加若干组 */
	if (1) {
		remove(cfgname); //删除文件
		//cout << "调试中：cfgname=" << cfgname << endl;
		fcfg.open(cfgname, judge, OPEN_OPT_RDWR);//打开文件
		if (judge == -1) {
	...
	if (1) {
		...
		fcfg.open(cfgname, judge, OPEN_OPT_RDWR);
		if (judge == -1) {
			cout << "无法打开配置文件(" << cfgname << ")." << endl;
			return -1;
		}
...
int main()
{
	...
	struct cfg_item item[] =//item的结构体数组定义并且初始化后，但是还要对union进行初始化
	{
